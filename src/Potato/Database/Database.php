...
class Database {
    ...
    public function where(string $table, array | string $where): array {
        $whereString = $this->convertWhere($where);
        $stmt = $this->query("SELECT * FROM $table WHERE $whereString", $where);
        return $stmt->fetchAll();
    }

    public function byId(string $table, string | int $id, string $idField = 'id'): ?stdClass {
    ...
    public function delete(string $table, array | string $where): PDOStatement {
        $whereString = $this->convertWhere($where);
    ...
    private function convertWhere(array | string $where): string {
        if (is_array($where)) {
            $pairs = array_map(function ($key) {
                return "$key = :$key";
            }, array_keys($where));
